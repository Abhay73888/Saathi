...
function subtractOne(a, b) {
    ...
    const pieces = [];
    if (b.start > a.start)
        pieces.push({ start: a.start, end: new Date(Math.min(b.start.getTime(), a.end.getTime())) });
    if (b.end < a.end)
        pieces.push({ start: new Date(Math.max(b.end.getTime(), a.start.getTime())), end: a.end });
    return pieces;
}
export function subtractIntervals(avail, blocked) {
...
export function computeFreeSlots({ recurring, overrides, bookings, durationMinutes, }) {
...
export function isWindowBookable(free, start, end) {
