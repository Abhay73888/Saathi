...
export class HeuristicRecommendationStrategy {
    weights;
    ...
    score(c, prefs) {
        ...
        const activities = prefs.experienceSlugs?.length
            ...
            : 0;
        let location = 0;
        if (prefs.city && c.city && prefs.city.toLowerCase() === c.city.toLowerCase())
            location = 1;
        ...
        const quality = clamp01((c.ratingAvg / 5) * 0.6 + c.responseRate * 0.25 + c.completionRate * 0.15);
        ...
        const raw = w.interests * interests +
    ...
    rank(companions, prefs) {
        return companions
            .map((c) => this.score(c, prefs))
            .sort((a, b) => b.score - a.score);
    }
}
export const recommendationEngine = new HeuristicRecommendationStrategy();
