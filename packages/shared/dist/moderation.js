...
const OFF_PLATFORM_PAYMENT = /\b(upi|gpay|google\s?pay|paytm|phonepe|phone\s?pe|bhim|net\s?banking|bank transfer|neft|imps)\b/i;
const PAY_OUTSIDE = /\b(pay me|pay outside|pay (you )?(direct|outside|privately)|avoid (the )?(fee|platform)|skip the fee|cash (only|payment|please))\b/i;
const THREAT_KEYWORDS = /\b(kill|beat you|hurt you|rape|threat|harm you|kidnap|bomb|police complaint|expose you|blackmail)\b/i;
const ESCORT_SOLICITATION = /\b(escort|call girl|call boy|paid sex|sexual|massage with happy ending|hotel room meetup|intimate service|hookup|one night stand|ons|sex chat|nudes|sensual massage|room service companion)\b/i;
const KEEP_ON_PLATFORM_WARNING = 'Keep payments and contact details on Saath — you stay protected and our safety team can help if something goes wrong.';
const SOLICITATION_WARNING = 'Saath is strictly for platonic, social companionship in public spaces. Escort or adult services are strictly prohibited and will lead to account termination.';
export function moderateMessage(text, context) {
    const signals = [];
    let riskPoints = 0;
